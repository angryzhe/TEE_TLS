...
static bool ecdsa_verify(sev_sig *sig, EVP_PKEY **pub_evp_key, uint8_t *digest, size_t length)
{
	...
	if (!ecdsa_sig) {
		RATS_ERR("failed to allocate ECDSA_SIG structure\n");
		goto err;
	}

	if (ECDSA_SIG_set0(ecdsa_sig, r, s) != 1) {
		RATS_ERR("failed to set the ecdsa_sig\n");
		goto err;
	}
		       ...
		       SHA_TYPE hash_algo, bool pss)
{
	...
	if (pss) {
		...
		if (RSA_verify_PKCS1_PSS(rsa_pub_key, hash,
					 ...
					 decrypted, -2) != 1) {
			RATS_ERR("failed to verify rsa with pss\n");
			goto err;
		}
	} else {
		/* Verify the data */
		if (RSA_verify((hash_algo == SHA_ALGO_256) ? NID_sha256 : NID_sha384, hash,
			...
			RATS_ERR("failed to verify rsa without pss\n");
			goto err;
		}
	}
		    ...
		    const SEV_SIG_ALGO algo)
{
	...
	case SEV_SIG_ALGO_ECDH_SHA256:
	case SEV_SIG_ALGO_ECDH_SHA384:
		RATS_ERR("unsupported ECDH signing\n");
		return false;
	default:
		RATS_ERR("invalid signing algo\n");
		return false;
	}
