...
__attribute__((unused)) static bool validate_usage(uint32_t usage)
{
	...
	case SEV_USAGE_CEK:
		return true;
	default:
		RATS_ERR("unsupported sev usage\n");
	}
...
static bool validate_body(const sev_cert *cert)
{
	...
	if ((cert->version == 0) || (cert->version > SEV_CERT_MAX_VERSION)) {
		RATS_ERR("invalid sev certificate version %d\n", cert->version);
		return false;
	}
...
static int compile_public_key_from_certificate(const sev_cert *cert, EVP_PKEY *evp_pub_key)
{
	...
	case SEV_SIG_ALGO_ECDH_SHA384:
		...
		break;
	default:
		RATS_ERR("unsupported sev sig algo\n");
		goto err;
	}
			       ...
			       EVP_PKEY *parent_signing_key)
{
	...
	case SEV_SIG_ALGO_ECDH_SHA384:
		...
		break;
	default:
		RATS_ERR("unsupported signing key algo! %x\n", parent_cert->pub_key_algo);
		return false;
	}
	...
	for (i = 0; i < SEV_CERT_MAX_SIGNATURES; i++) {
		if ((parent_cert->pub_key_algo == SEV_SIG_ALGO_RSA_SHA256) ||
			...
			if (!rsa_pub_key) {
				RATS_ERR("bad parent signing key\n");
				break;
			}
...
{
	RATS_DEBUG("child_cert %p, parent_cert1 %p, parent_cert2 %p\n", child_cert, parent_cert1,
		   parent_cert2);
	...
	for (i = 0; i < numSigs; i++) {
		...
		if (!validate_signature(child_cert, parent_cert[i], parent_pub_key[i])) {
			RATS_ERR("failed to validate signature\n");
			goto err;
		}
	}
