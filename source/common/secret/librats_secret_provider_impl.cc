...
LibratsTlsCertificateConfigProviderImpl::secret() const {
  ...
  // Wrapping c allocated buffers with std::string so they can be released before function return  
  std::string private_key;  
  private_key.assign(reinterpret_cast<char*>(p_private_key), private_key_size);  
    
  // 打印私钥的大小和完整内容 
  ENVOY_LOG(debug, "Private key size: {} bytes", private_key.size());
  std::ostringstream privateKeyStream;  
  privateKeyStream << private_key;  
  ENVOY_LOG(debug, "Private key content: {}", privateKeyStream.str());
  ...
  if (!this->certDerToPem(certificate, pem_cert)) {  
      std::cout << "Failed to convert DER certificate to PEM format." << std::endl;  
      return nullptr;  
  }  
    
  // 打印PEM格式证书的大小和完整内容  
  ENVOY_LOG(debug, "PEM certificate size: {}",  pem_cert.size());
  std::ostringstream pemCertStream;  
  pemCertStream << pem_cert;  
  ENVOY_LOG(debug, "PEM certificate content: {}", pemCertStream.str());
