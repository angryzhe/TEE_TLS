...
ValidationResults LibratsCertValidator::doVerifyCertChain(
  ...
  if (sk_X509_num(&cert_chain) != 1) {  
      stats_.fail_verify_error_.inc();  
      auto error = fmt::format("verify cert failed: depth of cert chain should be 1, but got {}",  
  ...
  certStream << leaf_cert; 
  std::string certStr = certStream.str();  
  ENVOY_LOG(debug, "Leaf Certificate Info: {}", certStr);
  ENVOY_LOG(debug, "Leaf Certificate Size: {} bytes", certStr.size());
